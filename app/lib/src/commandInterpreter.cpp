...
namespace cmd_interpreter {
    ...
    const std::string HELP_MESSAGE = "List of available instructions:\n"
                                     "> Linda Commands:\n"
                                     ">> thread_id:output(tuple), e.g. 1:output(string=\"ok\", float =1.1)\n"
                                     ">> thread_id:input((tuple_pattern), timeout), e.g. 2:input((string:\"no\", "
                                     "integer:*), 100)\n"
                                     ">> thread_id:read((tuple_pattern), timeout), e.g. 3:read((integer:<5), 200)\n"
